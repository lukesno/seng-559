...
export function AskingScreen({ questions, sendAnswers, timer }) {
  ...
  useEffect(() => {
    ...
    if (timer === 0 && !answersSubmitted) {
      ...
      if (answers.some(answer => answer === '')) {
        ...
        const updatedAnswers = answers.map(answer => answer === '' ? 'failed to respond' : answer);
  ...
  return (
    <div className="min-h-screen bg-indigo-900 text-white flex flex-col items-center justify-center p-5 relative">
      {/* Timer */}
      <div className={`absolute top-5 right-5 text-3xl font-bold ${timer <= 5 ? 'animate-ping' : ''}`}>
        Timer: {timer}
      </div>

      <h1 className="text-3xl font-bold text-purple-300 mb-4">Asking Screen</h1>
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg text-center">
        <h3 className="text-lg font-medium text-green-400 mb-4">Question {currentQuestionIndex + 1}</h3>
        <p className="text-white text-md mb-4">{questions[currentQuestionIndex]}</p>
        <input
          type="text"
        ...
        <button
          className={`mt-4 px-6 py-2 rounded-md font-semibold transition duration-300 ease-in-out ${
            answersSubmitted ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-700 text-white hover:shadow-lg'
          }`}
