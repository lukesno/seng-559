...
export function WaitingScreen({ isLeader, roomID, users, sendStartGame, username }) {
  ...
  return (
    <div className="min-h-screen bg-indigo-900 text-white flex flex-col items-center justify-center space-y-5">
      ...
      <div className="text-lg w-full max-w-md">
        ...
        <div className="grid grid-cols-2 gap-4 p-4 bg-gray-800 rounded-lg">
          {users.map((user, index) => (
            <div 
              key={index} 
              className={`flex justify-center items-center p-2 bg-gray-700 rounded-md m-1 hover:bg-gray-600 transition-all duration-300 ease-in-out ${user?.username === username ? 'border-2 border-green-400' : ''}`}
      ...
      <div className="flex space-x-4">
        {isLeader && (
          <button
            className={`px-6 py-2 rounded-full font-semibold transition duration-300 ease-in-out shadow-lg hover:shadow-xl ${canStartGame ? 'bg-green-500 hover:bg-green-700 ' : 'bg-gray-400 cursor-not-allowed'}`}
