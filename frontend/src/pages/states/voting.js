...
export function VotingScreen({ voteQuestion, voteAnswers, sendVote, timer, username }) {
  ...
  return (
    <div className="min-h-screen bg-indigo-900 text-white flex flex-col items-center justify-center p-5 relative space-y-6">
        ...
        {voteAnswers.map((answer, index) => (
          <button
            ...
            className={`py-2 px-4 md:px-8 rounded shadow-lg transition duration-300 ease-in-out w-full ${
              canVote ? 'bg-green-500 hover:bg-green-700 text-white font-bold hover:shadow-xl' : 'bg-gray-400 text-gray-600 cursor-not-allowed'
            }`}
